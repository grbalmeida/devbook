<div class="form-inline mb-3 row pl-3">
	<label>Data de nascimento</label>
	<div class="w-100 mb-2"></div>
		<select class="col-sm-12 mr-2 mb-1 col-md-3 form-control {{ session('date_error') ? 'is-invalid' : '' }}" name="day">
			@foreach($days as $day)
				<option value="{{ $day }}" {{ old('day') == $day ? 'selected' : '' }} @if(!empty($user)) @if(substr($user->birthday, 8) == $day) selected @endif @endif>{{ $day }}</option>
			@endforeach
		</select>
		<select class="col-sm-12 mr-2 mb-1 col-md-3 form-control {{ session('date_error') ? 'is-invalid' : '' }}" name="month">
		...
		<select class="col-sm-12 mr-2 mb-1 col-md-3 form-control {{ session('date_error') ? 'is-invalid' : '' }}" name="year">
